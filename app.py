...
#application specs
app = Flask(__name__)
app.config['HOST_FOLDER'] = HOST_FOLDER
...
def data_curator(path, imputation_method_id, outlier_detection_method_id, sim_method_id):
    ...
    temp_excel_path = "temp_data.xlsx"

    if path.endswith('.csv'):
        df = pd.read_csv(path)
        features = df.columns
        save_to_excel(df, temp_excel_path)
        wb = load_with_openpyxl(temp_excel_path)
        cleanup_temp_files(temp_excel_path)
        ...
        features = D['features']
        values = list(D['data'])
        df = pd.DataFrame(columns=features, data=values)
        save_to_excel(df, temp_excel_path)
        wb = load_with_openpyxl(temp_excel_path)
        cleanup_temp_files(temp_excel_path)
...
def index():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(os.path.join(app.config['HOST_FOLDER'], filename))
            try:
                ...
                data_curator(os.path.join(app.config['HOST_FOLDER'], filename), 
                ...
                os.remove(os.path.join(app.config['HOST_FOLDER'], filename))
